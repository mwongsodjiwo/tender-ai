<script lang="ts">
	...
	import ContractSettings from '$lib/components/contract/ContractSettings.svelte';
	...
	function getSectionDescription(key: string): string {
		...
		return templateSections.find((s) => s.key === key)?.description ?? '';
	}
...
<svelte:head><title>Conceptovereenkomst — {project.name} — Tendermanager</title></svelte:head>
...
<div class="fixed inset-0 z-[60] flex flex-col bg-[#F5F5F5]">
	...
	<EditorToolbar bind:focusedEditor bind:showSearch bind:zoomLevel {activeCommentsCount} isCommentsSidebarActive={showRightSidebar} on:toggleSearch={() => showSearch = !showSearch} on:toggleComments={toggleCommentsSidebar} />
	{#if showSearch}<EditorSearchBar {editorComponents} {artifacts} {sectionElements} hasReplace={false} on:close={() => showSearch = false} />{/if}
	...
	<div class="flex min-h-0 flex-1 overflow-hidden">
		...
		<div class="flex min-w-0 flex-col overflow-y-auto" role="region" aria-label="Contractinhoud" style="width: {showRightSidebar ? '60%' : '80%'};" bind:this={documentScrollContainer} on:scroll={handleScroll}>
			<div class="document-scroll-inner mx-auto py-8" style="transform: scale({(zoomLevel / 100) * 1.25}); transform-origin: top center; --editor-font-size: {fontSize}pt;">
				{#each artifacts as artifact, index (artifact.id)}
					<div bind:this={sectionElements[artifact.id]} class="relative scroll-mt-4" id="section-{artifact.id}" on:focusin={() => handleEditorFocus(artifact.id)} on:mouseup={() => handleTextSelection(artifact.id)} role="none">
						...
						<div class="document-paper mb-10 rounded bg-white"><TiptapEditor bind:this={editorComponents[artifact.id]} content={sectionContents[artifact.id] ?? artifact.content ?? ''} placeholder="Begin hier met het bewerken van het artikel..." showToolbar={false} on:change={(e) => { sectionContents[artifact.id] = e.detail; sectionContents = sectionContents; }} /></div>
				...
				{:else}
					<div class="flex items-center justify-center py-16"><div class="text-center"><svg class="mx-auto h-12 w-12 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg><h3 class="mt-3 text-sm font-semibold text-gray-900">Geen artikelen beschikbaar</h3><p class="mt-1 text-sm text-gray-500">Voltooi eerst de briefing om de conceptovereenkomst te laten genereren.</p></div></div>
				{/if}
			</div>
		</div>
...
<MobileStepper {artifacts} {currentSectionIndex} sectionLabel="Artikel" on:navigate={(e) => scrollToSection(e.detail)} />
