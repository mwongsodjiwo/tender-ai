<script lang="ts">
	...
	function getSectionDescription(sectionKey: string): string {
		...
		const template = documentType.template_structure.find(
			(s: { key: string; description?: string }) => s.key === sectionKey
		);
	...
	function toggleCommentsSidebar() {
		if (showRightSidebar && sidebarTab === 'comments') {
			// Already showing comments — hide sidebar
	...
	function handleTextSelection(artifactId: string) {
		...
		// Get position of selection for popup
	...
	async function submitAiRewrite() {
		...
		const response = await fetch(`/api/projects/${project.id}/section-chat`, {
			...
			body: JSON.stringify({
				...
				message: `Herschrijf het volgende stuk tekst volgens deze instructie: "${aiRewriteInput.trim()}"\n\nTekst om te herschrijven:\n"${text}"\n\nGeef ALLEEN de herschreven tekst terug, zonder uitleg.`
			})
		});
			...
			if (comp) {
				...
				if (ed) {
					ed.chain()
						...
						.insertContent(result.data.content)
						.run();

					// Update tracked content
					const html = ed.getHTML();
					sectionContents[artifactId] = html;
	...
	// Filter comments for current section or all
	...
	$: steps = artifacts.map((a, i) => {
		...
		if (a.status === 'approved') {
			status = 'completed';
		} else if (a.status === 'generated' || a.status === 'review' || (a.status === 'draft' && a.content)) {
			status = i === currentSectionIndex ? 'active' : 'active';
		} else {
			status = i === currentSectionIndex ? 'active' : 'pending';
		}
		return { label: a.title, status };
	});
	...
	const ZOOM_BASE = 1.25; // 100% displayed = 125% actual scale
	const FONT_SIZES = [
		{ value: '9', label: '9pt' },
		{ value: '10', label: '10pt' },
		{ value: '11', label: '11pt' },
	...
	// Store references to each TiptapEditor component
	...
	$: if (focusedEditor) {
		// This reactive block re-runs when focusedEditor changes.
		// We set up a transaction listener.
		const ed = focusedEditor;
		const handler = () => updateToolbarFromEditor(ed);
		ed.on('transaction', handler);
		updateToolbarFromEditor(ed);
		// Cleanup: will run when this block re-triggers
	}

	// Toolbar actions (delegated to focused editor)
	...
	function handleGlobalKeydown(event: KeyboardEvent) {
		// Ctrl+F or Cmd+F to open search (like Word)
		if ((event.ctrlKey || event.metaKey) && event.key === 'f') {
			event.preventDefault();
			if (!showSearch) {
				toggleSearch();
			} else {
				// Already open — focus and select all text in search input
				searchInput?.focus();
				searchInput?.select();
			}
		}
	...
	<header class="flex shrink-0 items-center justify-between border-b border-gray-200 bg-white px-4 py-2.5 sm:px-6">
		<div class="flex items-center gap-4">
			<Breadcrumbs items={[
				...
				{ label: 'Documenten', href: `/projects/${project.id}/documents` },
	...
	<div class="flex min-h-0 flex-1 overflow-hidden">
		...
		<aside class="hidden shrink-0 flex-col border-l border-gray-200 bg-white xl:flex" style="width: 20%;">
			...
			{#if sidebarTab === 'comments'}
				<div class="flex flex-1 flex-col overflow-hidden">
					<!-- New comment input (appears when adding a comment) -->
...
<div class="fixed bottom-0 left-0 right-0 z-[60] border-t border-gray-200 bg-white p-2 lg:hidden">
	<div class="flex items-center justify-between px-2">
		<span class="text-xs text-gray-500">
			Sectie {currentSectionIndex + 1} van {totalCount}
		</span>
				...
				<button
					...
					class="h-2 w-2 rounded-full {i === currentSectionIndex
						...
						: artifacts[i].status === 'approved'
							? 'bg-success-600'
							: artifacts[i].status === 'generated' || (artifacts[i].status === 'draft' && artifacts[i].content)
								? 'bg-blue-400'
								: 'bg-gray-300'}"
...
<style>
	...
	.document-paper :global(.tiptap-editor-wrapper .tiptap) {
		padding: 60px 72px; /* ~2cm margins like a real A4 document */
		min-height: 1123px;
