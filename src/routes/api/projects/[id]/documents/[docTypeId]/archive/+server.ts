...
// Archive all artifacts for a document type in a project
...
export const POST: RequestHandler = async ({ params, locals }) => {
	...
	const { data, error } = await updateArtifactsArchiveStatus(
		supabase, params.id, params.docTypeId, 'archived'
	);
	...
	await logArchiveAudit(supabase, {
		...
		newStatus: 'archived', affectedCount: data?.length ?? 0
	});

	return apiSuccess({
		message: 'Document gearchiveerd',
